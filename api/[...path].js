...
const innerHandler = async (req, res) => {
  ...
  // ── Tone Guardian (valida voz de marca en outputs) ──────────────────────────
  if (path.startsWith('/api/tone-guardian/')) {
    let tgBody = req.body;
    if (tgBody === undefined && (m === 'POST' || m === 'PATCH')) {
      try {
        const chunks = [];
        for await (const c of req) chunks.push(c);
        const raw = Buffer.concat(chunks).toString('utf-8');
        tgBody = raw ? JSON.parse(raw) : {};
      } catch {
        tgBody = {};
      }
    }
    const tgCtx = await getSessionFromReq(req).catch(() => null);
    try {
    ...
    if (path === '/api/auth/me' && m === 'GET') {
      getSessionFromReq(req)
        .then((ctx) => {
          ...
          import('./_achievements.js').then((a) => a.onUserLogin(ctx.user.id, ctx.user.lastActiveAt)).catch(() => {});
        })
        .catch(() => {});
    }
    try {
        ...
        getSessionFromReq(req)
          .then((ctx) => {
            ...
            import('./_achievements.js')
              .then((a) => a.onPostPublished(ctx.user.id, { afterMidnight, friday13: isFriday13 }))
              .catch(() => {});
          })
          .catch(() => {});
      }
          ...
          try {
            const parsed =
              typeof llmResult === 'string'
                ? JSON.parse(llmResult.replace(/^```json\s*/i, '').replace(/```\s*$/i, ''))
                : llmResult;
  ...
  if (path === '/api/format/optimize' && m === 'POST') {
    ...
    const specs = {
      ...
      'instagram-story': {
        w: 1080,
        h: 1920,
        safeZone: '70% (top 30% + bottom 30% reserved)',
        dpi: 72,
        format: 'jpg/png/mp4',
      },
  ...
  if (path.startsWith('/api/achievements')) {
    ...
    if (path === '/api/achievements/snapshot' && m === 'GET') {
      return ok(res, {
        ...
        byCategory: {},
        byRarity: {
          común: DEMO_UNLOCKED.filter((a) => a.rarity === 'común').length,
          rara: DEMO_UNLOCKED.filter((a) => a.rarity === 'rara').length,
          épica: 0,
          legendaria: 0,
          mítica: 0,
        },
    ...
    if (path === '/api/achievements/next' && m === 'GET') {
      ...
      return ok(
        res,
        DEMO_ACHIEVEMENTS.filter((a) => !unlockedIds.has(a.id) && !a.hidden)
          .slice(0, 5)
