...
const tickForUser = async (userId, planId) => {
  ...
  // Run autopilot (recomendaciones — no ejecuta nada por sí solo)
  ...
  try {
    ...
    recommendedActions = result.recommendedActions?.length || 0;
  } catch (err) {
    return { userId, error: String(err.message || err) };
  }

  // Dispatch de calendario editorial (Pro+) — publica de verdad los slots
  // vencidos. Gate propio (mismo feature que el generador de calendario),
  // no consume el cap de ticks del autopilot gratuito.
  let calendarDispatch = null;
  if (hasFeature(planId, 'contentGeneration.calendarAutoGenerated')) {
    try {
      const { dispatchDueCalendarSlots } = await import('./_calendarPlanner.js');
      calendarDispatch = await dispatchDueCalendarSlots({ userId });
    } catch (err) {
      calendarDispatch = { error: String(err?.message || err) };
    }
  }

  return { userId, planId, executed: true, recommendedActions, calendarDispatch };
};
