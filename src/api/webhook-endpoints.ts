 ...
 */
export const deliverWebhook = async (url: string, payload: unknown): Promise<boolean> => {
  for (let attempt = 0; attempt <= MAX_RETRIES; attempt++) {
    try {
      const response = await fetch(url, {
        ...
        // fetch's RequestInit has no `timeout` field -- that option was
        // silently ignored (extra properties don't error), so this never
        // actually timed out and would hang until the OS/TCP-level
        // timeout if a target server never responded. AbortSignal.timeout
        // is the real, fetch-native way to do this.
 ...
 */
export const queueWebhookDelivery = async (userId: string, eventType: string, data: unknown): Promise<void> => {
  try {
 ...
 */
const attemptWebhookDelivery = async (deliveryId: string, url: string, payload: unknown): Promise<void> => {
  let retryCount = 0;
  ...
  while (retryCount <= MAX_RETRIES) {
    ...
    if (retryCount <= MAX_RETRIES) {
      ...
      await query('UPDATE webhook_deliveries SET retry_count = $1, last_retry_at = NOW() WHERE id = $2', [
        retryCount,
        deliveryId,
      ]);
    }
  ...
  // Final failure
  await query(`UPDATE webhook_deliveries SET status = $1, error_message = $2 WHERE id = $3`, [
    'failed',
