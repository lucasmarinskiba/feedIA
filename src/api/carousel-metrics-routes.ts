...
import { carouselMetricsService } from '../services/carousel-metrics-service.js';
...
router.get('/api/carousels/:carousel_id/metrics', async (req: Request, res: Response) => {
  try {
    const carouselId = req.params.carousel_id as string;
    const metrics = await carouselMetricsService.getCarouselMetrics(carouselId);
    return res.json({ carousel_id: carouselId, metrics, timestamp: new Date().toISOString() });
  } catch (err) {
...
router.get('/api/carousels/:carousel_id/metrics/history', async (req: Request, res: Response) => {
  try {
    ...
    const history = await carouselMetricsService.getCarouselHistory(carouselId, days);
    return res.json({ carousel_id: carouselId, days, data: history, count: history.length });
  } catch (err) {
...
router.get('/api/users/:user_id/metrics/summary', async (req: Request, res: Response) => {
  try {
    ...
    const summary = await carouselMetricsService.getUserEngagementSummary(userId);
    return res.json({ user_id: userId, summary });
  } catch (err) {
...
router.get('/api/users/:user_id/carousels/top', async (req: Request, res: Response) => {
  try {
    const userId = req.params.user_id as string;
    const topCarousels = await carouselMetricsService.getUserTopCarousels(userId);
    return res.json({ user_id: userId, top_carousels: topCarousels, count: topCarousels.length });
  } catch (err) {
...
router.get('/api/carousels/:carousel_id/metrics/breakdown', async (req: Request, res: Response) => {
  try {
    ...
    const breakdown = await carouselMetricsService.getCarouselEventBreakdown(carouselId, days);
    return res.json({ carousel_id: carouselId, days, events: breakdown });
  } catch (err) {
