...
class CarouselAnalyticsService {
  ...
  async getUserAnalytics(userId: string): Promise<UserAnalytics> {
    ...
    const carouselStmt = db.prepare(`SELECT COUNT(DISTINCT id) as total FROM carousels WHERE user_id = ?`);
    ...
    const platformStmt = db.prepare(`SELECT metadata FROM carousels WHERE user_id = ?`);
    ...
    const trend =
      (lastWeekViews?.views || 0) > (prevWeekViews?.views || 0)
        ? 'up'
        : (lastWeekViews?.views || 0) < (prevWeekViews?.views || 0)
          ? 'down'
          : 'flat';
  ...
  async compareCarousels(carouselIds: string[]): Promise<
    Array<{
      carouselId: string;
      title: string;
      views: number;
