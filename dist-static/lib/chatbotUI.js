import{apiSafe as E}from"./api.js";const i=t=>document.getElementById(t),l=t=>String(t??"").replace(/[&<>"]/g,a=>({"&":"&amp;","<":"&lt;",">":"&gt;",'"':"&quot;"})[a]),j=[{emoji:"\u23F0",text:"\xBFCu\xE1l es la mejor hora para publicar?"},{emoji:"\u{1F0CF}",text:"Dame una idea de carrusel viral sobre IA"},{emoji:"\u{1F4CA}",text:"Analiz\xE1 mi marca en Instagram"},{emoji:"\u{1F3AC}",text:"5 hooks para un reel de automatizaci\xF3n"},{emoji:"\u{1F4C8}",text:"\xBFC\xF3mo subo mi engagement rate?"},{emoji:"\u{1F5D3}\uFE0F",text:"Estrategia de contenido para 30 d\xEDas"},{emoji:"\u{1F3AF}",text:"\xBFQu\xE9 formato me funciona mejor?"},{emoji:"\u{1F6E1}\uFE0F",text:"\xBFHay alg\xFAn riesgo de shadowban hoy?"}],S=[{e:"\u{1F3A8}",n:"Nova",t:"Dise\xF1adora \u2014 arma carruseles, reels y stories con tu marca"},{e:"\u270D\uFE0F",n:"L\xEDa",t:"Copywriter \u2014 escribe captions, hooks y respuestas con voz de marca"},{e:"\u{1F6E1}\uFE0F",n:"Gard",t:"Compliance \u2014 valida tono, riesgo, hashtags y pol\xEDticas antes de publicar"},{e:"\u{1F680}",n:"Luca",t:"Publisher \u2014 sube los posts a Instagram (API o Computer Use)"},{e:"\u{1F4C8}",n:"Mira",t:"M\xE9tricas \u2014 analiza performance y programa boosts cuando conviene"}];let T=0;const x=(t,a,o=8)=>{const s=`cblg-${++T}`;return`
  ...
  <div class="chatbot-sugg-grid">
    ${j.map(t=>`<button class="chatbot-sugg" data-q="${l(t.text)}">${t.emoji} ${l(t.text)}</button>`).join("")}
  </div>`,y=()=>{const t=i("chatbot-log");if(t){if(!r.length){t.innerHTML=I();return}t.innerHTML=r.map(q).join(""),g&&t.insertAdjacentHTML("beforeend",H()),t.scrollTop=t.scrollHeight,t.querySelectorAll(".chatbot-offline-retry[data-q]").forEach(a=>{a.addEventListener("click",()=>{r.at(-1)?.content==="Sin conexi\xF3n"&&r.pop(),b(a.dataset.q)})}),t.querySelectorAll(".chatbot-tool-chip[data-route]").forEach(a=>{a.addEventListener("click",o=>{o.preventDefault();const s=a.dataset.route;s&&(u(),A(s))})})}},w=()=>{const t=i("chatbot-sugg-box");t&&(t.style.display=r.length===0?"block":"none")},k=()=>{const t=i("chatbot-panel");t&&(v=!0,t.classList.add("open"),t.setAttribute("aria-hidden","false"),y(),w(),setTimeout(()=>i("chatbot-input")?.focus(),60))},u=()=>{v=!1;const t=i("chatbot-panel");t&&(t.classList.remove("open"),t.setAttribute("aria-hidden","true"),window.closeVoiceOverlay?.())},C=()=>{r=[],y(),w()},b=async t=>{const a=(t??"").trim();if(!a||g)return;r.push({role:"user",content:a,ts:Date.now()});const o=i("chatbot-input");o&&(o.value="",o.style.height="auto");const s=i("chatbot-send");s&&(s.disabled=!0),g=!0,w(),y();const{data:e,error:n}=await E("/api/assistant/chat",null,{method:"POST",body:{message:a,history:r.slice(-10,-1)}});g=!1,s&&(s.disabled=!1),n||!e?r.push({role:"assistant",html:`<div class="chatbot-offline-msg">
      ...
      </div>`,content:"Sin conexi\xF3n",ts:Date.now()}):r.push({role:"assistant",content:e.reply??"",html:e.replyHtml,tools:e.tools??[],ts:Date.now()}),y()};export const initChatbotUI=({navigate:t})=>{A=t||(e=>{window.location.hash=`#${e}`}),i("chatbot-fab")?.addEventListener("click",()=>{v?u():k()}),i("chatbot-close")?.addEventListener("click",u),i("chatbot-clear")?.addEventListener("click",C),i("chatbot-form")?.addEventListener("submit",e=>{e.preventDefault(),b(i("chatbot-input")?.value??"")}),document.addEventListener("click",e=>{const n=e.target.closest?.(".chatbot-sugg");n&&n.dataset.q&&b(n.dataset.q)});const a=i("chatbot-input");a?.addEventListener("input",()=>{a.style.height="auto",a.style.height=Math.min(a.scrollHeight,100)+"px"}),a?.addEventListener("keydown",e=>{e.key==="Enter"&&!e.shiftKey&&(e.preventDefault(),b(a.value))}),document.addEventListener("keydown",e=>{e.key==="Escape"&&v&&u()});const o=i("chatbot-mic-btn"),s=typeof window<"u"?window.SpeechRecognition||window.webkitSpeechRecognition:null;if(o&&s){let e=null,n=!1;const p=()=>{n=!1,o.classList.remove("recording"),o.setAttribute("aria-label","Dictar mensaje");try{e?.stop()}catch{}};o.addEventListener("click",()=>{if(n){p();return}n=!0,o.classList.add("recording"),o.setAttribute("aria-label","Grabando\u2026 toc\xE1 para parar");const d=i("chatbot-input");e=new s,e.lang=navigator.language||"es-AR",e.continuous=!1,e.interimResults=!0,e.maxAlternatives=1;let f="",m=null;const $=()=>{clearTimeout(m),m=setTimeout(()=>{try{e?.stop()}catch{}},1800)};e.onresult=c=>{$();let L="";for(let h=c.resultIndex;h<c.results.length;h++)c.results[h].isFinal?f+=c.results[h][0].transcript+" ":L=c.results[h][0].transcript;d&&(d.value=(f+L).trim())},e.onend=()=>{clearTimeout(m),p();const c=(f||d?.value||"").trim();c&&c.split(/\s+/).filter(Boolean).length>=2&&b(c)},e.onerror=c=>{clearTimeout(m),c.error!=="aborted"&&(d&&(d.placeholder="Error de micr\xF3fono \u2014 verific\xE1 permisos"),setTimeout(()=>{d&&(d.placeholder="Preguntale lo que quieras a FeedIA\u2026")},3e3)),p()},$();try{e.start()}catch{p()}})}else o&&(o.style.display="none")},openChatbot=k;window.closeChatbotPanel=u;
